...
object ConfigToolkit {
    ...
    fun getWindowedResolution()=try {
       val (x,y)= Config.WindowedResolution.load().split('_', ignoreCase = false, limit = 2).map { it.toInt().takeIf { number -> number>0 }?:throw NumberFormatException("范围不对") }
Pair(x,y)
    }catch (e: Throwable){
        ExceptionDialog(
            e, true, """
1、NumberFormatException，IndexOutOfBoundsException：
    config.properties文件的WindowedResolution设置项，值的文本格式，或者范围不对：
        当前设置项的值为：${Config.WindowedResolution.load()}
