...
object GdxFont {
    ...
    private fun getCharacters()=
        try {
            ...
            Properties().apply {
                // 使用类加载器获取资源输入流
                val inputStream = GdxFont.javaClass.classLoader.getResourceAsStream("${CoreConfig.LanguageResourcePath.load()}.properties")
...
1、FileNotFoundException
     无法找到资源文件: ${CoreConfig.LanguageResourcePath.load()}.properties
...
其他错误为未知错误。该函数返回默认字符集。
     ...
     */
    fun getLightBitmap (config: FreeTypeFontGenerator.FreeTypeFontParameter.() -> Unit) =
        getBitmapFont(CoreConfig.FontLight.load()) { // 加载字体
     ...
     */
    fun getRegularBitmap (config: FreeTypeFontGenerator.FreeTypeFontParameter.() -> Unit) =
        getBitmapFont(CoreConfig.FontRegular.load()) {
     ...
     */
    fun getBoldBitmap (config: FreeTypeFontGenerator.FreeTypeFontParameter.() -> Unit) =
        getBitmapFont(CoreConfig.FontBold.load()) {
            characters = getCharacters()
            config(this)
        }
