...
object AllocateMusician{
     ...
     */
     fun getRandomSeat(instrument: Instrument): Musician?=   when(instrument){
        is GeneralInstrument -> Musicians.entries.map { it.musician }.filterIsInstance<GeneralSeat>().randomOrNull()

        is PercussionInstrument ->  Musicians.entries.map { it.musician }.filterIsInstance<PercussionSeat>().randomOrNull()?: Musicians.entries.map { it.musician }.filterIsInstance<GeneralSeat>().randomOrNull()//如果没有打击乐席位，就分配普通乐器顶替
    }
