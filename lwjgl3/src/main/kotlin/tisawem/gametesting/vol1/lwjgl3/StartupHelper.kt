...
class StartupHelper private constructor() {
    init {
        ExceptionDialog(UnsupportedOperationException(),false,"不支持创建实例：${this::class.qualifiedName}")
    }
    ...
    companion object {
        ...
        fun startNewJvmIfRequired(redirectOutput: Boolean = true): Boolean {
            ...
            if ("true" == System.getProperty(JVM_RESTARTED_ARG)) {
                ExceptionDialog(Throwable(),true,"There was a problem evaluating whether the JVM was started with the -XstartOnFirstThread argument.")
                return false
            }
            ...
            val javaExecPath = ProcessHandle.current().info().command().orElse(System.getProperty("java.home") + separator + "bin" + separator + "java")
            if (!File(javaExecPath).exists()) {
                ExceptionDialog(Throwable(),true,"A Java installation could not be found. If you are distributing this app with a bundled JRE, be sure to set the -XstartOnFirstThread argument manually!")
                return false
            }
            jvmArgs.apply {
                add(javaExecPath)
            ...
            if (mainClass == null) {
                ...
                mainClass = if (trace.isNotEmpty()) {
                    ...
                    ExceptionDialog(Throwable(),true,"The main class could not be determined.")
                    return false
                }
            }
            ...
            try {
                ...
                ExceptionDialog(e,false,"There was a problem restarting the JVM")
            }
            return true
        }
    }
